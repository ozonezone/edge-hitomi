  ...
  static async synchronize(): Promise<void> {
    ...
    while (nextIndex !== -1) {
      switch (this.#response[currentIndex]) {
        ...
        case "c": {
          this.#subdomainCodes.add(
            Number(this.#response.slice(currentIndex + 5, nextIndex - 1)),
          );
          break;
        }
        ...
        case "b": {
          this.#pathCode = this.#response.slice(
            currentIndex + 4,
            nextIndex - 2,
          );
          break;
  ...
  static getImageUri(
    ...
    if (!this.#subdomainCodes.size) {
      throw new HitomiError(
        ERROR_CODE.INVALID_VALUE,
        "getImageUri",
        "called after synchronize",
      );
    }

    if (
      !image[
        `has${extension[0].toUpperCase()}${
          extension.slice(1)
        }` as keyof HitomiImage
      ]
    ) {
      throw new HitomiError(
        ERROR_CODE.INVALID_VALUE,
    ...
    const imageHashCode: number = Number.parseInt(
      imageHashCodeParts[0] + imageHashCodeParts[1],
      16,
    );
    ...
    if (options?.isThumbnail) {
      ...
      path += `/${imageHashCodeParts[0]}/${imageHashCodeParts[1]}`;
      subdomain = (this.#subdomainCodes.has(imageHashCode) === this.#startsWithA
        ? "a"
        : "b") + "tn";
    } else {
      path = `${this.#pathCode}/${imageHashCode}`;
      subdomain = extension[0] +
        (this.#subdomainCodes.has(imageHashCode) === this.#startsWithA
          ? "1"
          : "2");
    }
    // Reference subdomain_from_url from /common.js
    return `${HTTPS}${subdomain}.${HITOMI_LA}/${path}/${image.hash}.${extension}`;
  }
}
